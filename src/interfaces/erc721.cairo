...
pub trait ICredenzaCredential<TContractState> {
    ...
    fn get_user_credentials(
        self: @TContractState, user: ContractAddress, offset: u32, limit: u32,
    ) -> Array<u256>;

    fn get_credentials_by_schema(
        self: @TContractState, schema_id: felt252, offset: u32, limit: u32,
    ) -> Array<u256>;

    fn get_active_credential_count(self: @TContractState, user: ContractAddress) -> u32;
    fn has_credential_type(
        self: @TContractState, user: ContractAddress, credential_type: felt252,
    ) -> bool;

    fn set_issuer_permissions(
        ref self: TContractState, issuer: ContractAddress, can_mint: bool, can_revoke: bool,
    );
