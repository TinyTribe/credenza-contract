...
fn setup_job_scenario() -> (
    ICredenzaDispatcher, IUserDispatcher, ContractAddress, ContractAddress, u256,
) {
...
fn test_recruiter_accept_applicant() {
    ...
    let expected_event = TestCredenzaContract::Event::CredenzaEvent(
        CredenzaComponent::Event::RecruiterAccepted(
            RecruiterAccepted { job_id, applicant, recruiter },
        ),
    );
    spy.assert_emitted(@array![(credenza.contract_address, expected_event)]);
}
...
fn test_applicant_confirm_job_offer() {
    ...
    let expected_event1 = TestCredenzaContract::Event::CredenzaEvent(
        CredenzaComponent::Event::ApplicantConfirmed(
            ApplicantConfirmed { job_id, applicant, recruiter },
        ),
    );
...
fn test_different_order_acceptance() {
    ...
    spy
        .assert_emitted(
            @array![(credenza.contract_address, event1), (credenza.contract_address, event2)],
        );
}
